...
def draw_from_dict(dicdata,RANGE, filename):
    ...
    plt.savefig("./dba"+str(filename).replace("./data","").replace(".txt",".jpg"))
...
def analysing(filename1,filename2):
    ...
    with open(filename2,"r") as off:
        lines2=off.readlines()
        off.close()
    print(len(lines2))
    ...
    for line in tqdm(lines1[1:]):
        ...
        try:
            ...
            dbm=int(eval(dbm))
            freqlist1[freq]+=abs(dbm-lastfreq1[freq])
            lastfreq1[freq]=dbm
        except:
            print(line)
            continue
    ...
    for line in tqdm(lines2[1:]):
        ...
        try:
            ...
            freq=int((int(eval(freq))-2400)/t)*t+2400
            dbm=int(eval(dbm))
            freqlist2[freq]+=abs(dbm-lastfreq2[freq])
            lastfreq2[freq]=dbm
        except:
            print(line)
            continue
    ...
    print(freqlist)
    draw_from_dict(freqlist,len(freqlist),"/dba.jpg")
